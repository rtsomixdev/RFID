...
const Requests: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (userStr) {
            ...
            setUser(currentUser);
            // Admin roleId = 1 or 2 (Adjust based on your DB)
    ...
    const checkStock = async (prodId: number) => {
        try {
            // เรียก API เช็คสต็อก (ต้องมั่นใจว่า Backend คืนค่า available มาถูกต้อง)
    ...
    const handleSubmit = async () => {
        ...
        // Validation: Stock Check (Front-end level)
        ...
        // ถ้าเป็นเปลี่ยนผ้า ต้องมีสาเหตุชำรุด
    ...
    const handleApprove = async (reqId: number, isApprove: boolean) => {
        ...
        Swal.fire({
            ...
            text: isApprove ? "ระบบจะตัดสต็อกและบันทึกประวัติทันที" : "คำร้องจะถูกยกเลิก",
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    // ส่ง API ไป Update Status
                    // Backend ควรจะมี Logic ตัดสต็อกเมื่อ Status เปลี่ยนเป็น Approved (2)
                    await axiosClient.put(`/Request/${reqId}`, {
                        requestId: reqId,
                        currentStatusId: isApprove ? 2 : 3, // 2=Approved, 3=Rejected
                        // ส่งข้อมูลเดิมกลับไปด้วยเพื่อความชัวร์ (หรือ Backend อาจจะไม่ต้องใช้ก็ได้)
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: `บันทึกสถานะ "${action}" เรียบร้อย`,
                        timer: 1500,
                        showConfirmButton: false
                    });
    ...
    return (
        <Box sx={{ pb: 5 }}>
            ...
            {/* 1. ส่วน Form (Create New Request) */}
                        ...
                        <Grid item xs={12} md={4}>
                            <FormLabel label="รายการผ้า" required>
                                <Select 
                                    ...
                                    onChange={e => setFormData({ ...formData, productId: e.target.value.toString() })} 
                                    displayEmpty
                                >
            ...
            <Card elevation={0} sx={{ borderRadius: 3, border: `1px solid ${theme.palette.divider}` }}>
                <Box sx={{ p: 2, borderBottom: `1px solid ${theme.palette.divider}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <IconButton onClick={fetchRequests} size="small"><Refresh /></IconButton>
                </Box>
                                ...
                                <TableRow key={req.requestId} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                    ...
                                    <TableCell align="center">
                                        <Stack direction="row" spacing={1} justifyContent="center">
                                            {/* แสดงปุ่มเฉพาะ Admin และสถานะรออนุมัติ */}
