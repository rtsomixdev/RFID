...
const RfidConnect: React.FC = () => {
  ...
  const [readerForm, setReaderForm] = useState({ name: '', ip: '', location: '' });
  ...
  const handleSaveReader = async () => {
    ...
    const payload = { readerName: readerForm.name, ipAddress: readerForm.ip || '-', location: readerForm.location, readerFunction: 'CHECK', isActive: true, currentMode: 'Normal' };
    try {
      ...
      setReaderForm({ name: '', ip: '', location: '' }); setIsEditingReader(false); setEditingReaderId(null); fetchReaders();
  ...
  const handleEditClick = (r: Reader) => {
    setReaderForm({ name: r.readerName, ip: r.ipAddress === '-' ? '' : r.ipAddress, location: r.location || '' });
    setIsEditingReader(true); setEditingReaderId(r.readerId); window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleCancelEdit = () => { setReaderForm({ name: '', ip: '', location: '' }); setIsEditingReader(false); setEditingReaderId(null); };
        ...
        <Box role="tabpanel" hidden={tabValue !== 0}>
          {tabValue === 0 && (
            <CardContent sx={{ p: 3 }}>
              ...
              {/* ซ่อนฟอร์มถ้าไม่มีสิทธิ์เขียน/แก้ไข */}
                  ...
                  <TableBody>
                    {readers.length === 0 ? (
                      ...
                      <TableRow key={r.readerId} hover selected={editingReaderId === r.readerId}>
                        ...
                        <TableCell sx={{ fontFamily: 'monospace', color: 'text.secondary' }}>{r.ipAddress && r.ipAddress !== '-' ? <Chip label={r.ipAddress} size="small" variant="outlined" /> : '-'}</TableCell>
        ...
        <Box role="tabpanel" hidden={tabValue !== 1}>
          {tabValue === 1 && (
            <CardContent sx={{ p: 3 }}>
              ...
              {/* ซ่อนฟอร์มสร้าง Tag ถ้าไม่มีสิทธิ์ */}
              {canWrite && (
